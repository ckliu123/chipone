...
uint8_t captest_frame[FRAME_SIZE_HAS_TAIL];
#endif
...
#define DATA_FORMAT_STR     "%-5u"
    ...
    for (r = 0; r < ROWS; r++)
    {
        ...
        for (c = 0; c < COLS; c++)
        {
            count += snprintf(line_buf + count, sizeof(line_buf) - count, DATA_FORMAT_STR, data[r * COLS + c]);
        }
        CTS_THP_LOGI("%s", line_buf);
    }
    CTS_THP_LOGI(SPLIT_LINE_STR);
...
static int cts_inspect_short(void)
{
    ...
    fail_nodes = cts_validate_tsdata("GND-short", rawdata, SHORT_TEST_MIN, INT_MAX);
    if (fail_nodes)
    {
    	r += 1;
    ...
    for (loopcnt = 0; loopcnt < SHORT_COLS_TEST_LOOP; loopcnt++)
    {
        ...
        fail_nodes = cts_validate_tsdata("Col-short", rawdata, SHORT_TEST_MIN, INT_MAX);
        if (fail_nodes)
        {
        	r += 1;
    ...
    for (loopcnt = 0; loopcnt < SHORT_ROWS_TEST_LOOP; loopcnt++)
    {
        ...
        if (loopcnt == SHORT_ROWS_TEST_LOOP - 1)  // only judge the last frame mbteng 20241129
        {
            ...
            fail_nodes = cts_validate_tsdata("Row-short", rawdata, SHORT_TEST_MIN, INT_MAX);
            if (fail_nodes)
            {
            	r += 1;
...
uint32_t cts_inspect(void)
{
    ...
    {
        CTS_THP_LOGI("Wait firmware to normal work failed");
    }
