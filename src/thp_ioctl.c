...
int thp_dev_open(void)
{
    ...
    //for reopen flow -- fsong,2026/2/26  ------ start ------
    if (s_dev_fd > 0) {
            CTS_THP_LOGE("fd is open, fd=%d", s_dev_fd);
            return 0;
    }
...
int thp_ioctl_set_irq(uint32_t enable)
{
    ...
    int ret = -1;
    // CTS_THP_LOGI("Enter, enable111111111=%d", enable);
    // //return 0;

    // mdelay(100);

    CTS_THP_LOGI("Enter, enable22222222=%d", enable);
